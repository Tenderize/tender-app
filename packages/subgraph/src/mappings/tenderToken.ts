...
import { addressIsContract, getProtocolIdByTenderTokenAddress, loadOrCreateUserDeployment, loadOrCreateUserDeploymentDay } from "./utils"
...
export function handleTransferEvent(transferEvent: Transfer): void {
    ...
    let config = Config.load(protocolId) as Config
    ...
    if(!addressIsContract(config, transferEvent.params.from)){
    ...
    if(!addressIsContract(config, transferEvent.params.to)){
