...
const JoinPool: FC<Props> = ({name, symbol, tokenBalance, tenderTokenBalance, tokenWeight, tenderTokenWeight, totalWeight, swapFee, tokenLpBalance, tenderLpBalance, lpShares}) => {
    ...
    const maxDeposit = (tenderToken: boolean) => {
        if (tenderToken) {
            ...
            setTokenInput(
                utils.formatEther(balBN.mul(tokenWeight).div(tenderTokenWeight))
            )
        } else {
            setTokenInput(utils.formatEther(tokenBalance||"0"))
            const balBN = BigNumber.from(tokenBalance.toString())
            setTenderInput(
                utils.formatEther(balBN.mul(tenderTokenWeight).div(tokenWeight))
            )
        }
      };
    ...
    console.log(singlePoolOut?.toString())

    const {state: approveTokenTx, send: approveUnderlyingTokens} = useContractFunction(contracts[name].token, 'approve')
    const approveTokens = (e: any) => {
        e.preventDefault()
        approveUnderlyingTokens(addresses[name].liquidity, utils.parseEther(tokenInput||"0"))
    }

    const {state: approveTenderTx, send: approveTenderTokens} = useContractFunction(contracts[name].tenderToken, 'approve')
    ...
    const {state: joinSwapExternAmountInTx, send: joinSwapExternAmountIn} = useContractFunction(contracts[name].liquidity, 'joinswapExternAmountIn')
    ...
    return (
        ...
        <Tabs onSelect={handleMulti} defaultActiveKey="multi" id="jointabs">
            ...
            <Form>
                <Form.Group controlId="tokenInput">
                    ...
                    <InputGroup>
                        <InputGroup.Prepend>
                            <InputGroup.Text id="basic-addon1">{`${utils.formatEther(calcWeight(tokenWeight)).substr(0,5)} %`}</InputGroup.Text>
                        </InputGroup.Prepend>
                        <Form.Control
                            value={tokenInput}
                ...
                <Form.Group controlId="tenderTokenInput">
                    ...
                    <InputGroup>
                        <InputGroup.Prepend>
                            <InputGroup.Text id="basic-addon1">{`${utils.formatEther(calcWeight(tenderTokenWeight)).substr(0,5)} %`}</InputGroup.Text>
                        </InputGroup.Prepend>
                        <Form.Control
                            value={tenderInput}
            ...
            <Tab eventKey='single' title="Single-Asset">
                <Form>
                    ...
                    <InputGroup className="mb-3">
                        ...
                        value={selectToken}
                        >
                        <Dropdown.Item eventKey={symbol} onSelect={() => handleSelectToken(symbol)}>{symbol}</Dropdown.Item>
                        <Dropdown.Item eventKey={`t${symbol}`} onSelect={() => handleSelectToken(`t${symbol}`)}>t{symbol}</Dropdown.Item>
                        </DropdownButton>
                        {
                            selectToken === symbol ?
                                <>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={addLiquidity}>Add Liquidity</Button>
